 ...
 *
 * **That sentence has now been paid, which is worth recording because it is the
 * cheapest kind of vindication.** `platform:diagnostics` arrived (ROADMAP §6b) and
 * with it a seventh code, `diagnostics-unreachable`. It was not a patch: `health`
 * went to `1.1.0` in the same change, for exactly the reason above — a member
 * running an older release folds these strings out of beats written by a member
 * running a newer one, and a code that release has never heard of has to be a
 * declared widening rather than a surprise. The trigger it names is still open; this
 * was the other reason a list of literals would have to move.
 */
 ...
 *                       — `platform:diagnostics` threw. That port is the one read
 *                         here whose *absence* is survivable, so this code has to be
 *                         distinguishable from the port never having been bound: a
 *                         device with no such binding reports `observed: false` from
 *                         `diagnostics()` and records no fault, while a device whose
 *                         binding refuses records this. Conflating the two would make
 *                         "this network did not grant the port" look like "the kernel
 *                         is failing", which sends an operator to the wrong console.
