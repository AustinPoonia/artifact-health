/**
 * What `health@1.3.0` promises, written once, as data.
 *
 ...
 * cannot do that.
 *
 * **That day arrived twice, and the two arrivals are why this contract is at `1.2.0`.**
 * The first was `platform:diagnostics@1`: `zone deaths` left `limits`, no dashboard
 * changed, and a shell rendering `limits()` stopped drawing one asterisk and started
 * drawing a number. Two of the original four rows stayed with *rewritten reasons* rather
 * than the same reason, and that is the part the design did not anticipate — a port can
 * make a category reachable while leaving the sentence about it wrong in a new way.
 *
 * The second is this version, and it is the more interesting of the two because nothing
 * about the port was the problem. `refusals` stayed because the kernel's journal filed a
 * refusal under the same word as a moved pin; that word was split at the call sites,
 * `platform:diagnostics@2` names the split, and the row is out. `index.js` has each one.
 *
 * **Why a minor and not a major, when the port it reads went to `2.0.0`.** Nothing here
 * that a caller depends on changed shape: `diagnostics()` still answers `observed`,
 * `dropped` and a list of `{ kind, count }` rows, and that list was already declared as
 * pass-through — "a kind this release has never heard of is reported under its own name".
 * A consumer at `^1.0.0` gets two more rows and reads them exactly as it would have read
 * a kind from a newer runtime, which is the case that shape was chosen for. What moved is
 * `limits()`, and it moved in the direction the contract already promises it moves: the
 * list shrinks when the platform makes something reachable, and shrinking it is not a
 * breaking change to anybody rendering it. The port range widened to `^2.0.0`, which is a
 * fact about what this artifact needs and not about what it offers.
 *
 * **`1.3.0` is the third arrival and it is not a port's.** The first two were the
 * platform making something reachable and `limits` shrinking. This one is a row that had
 * been missing from `limits` since the beginning, and a field to close it. `reached` is
 * monotone — one entry from a member is on disk forever — so a member that stops
 * replicating after first contact never leaves `reached`, never enters `silent` and never
 * sets `degraded`. Every one of those fields is doing exactly what it is declared to do;
 * the defect was that the question an operator actually asks had no field at all and no
 * row saying so.
 *
 * `age` is that field, and adding it forced the decision the beat had been avoiding. A
 * currency reading needs the feed to move, suppression on content alone means a healthy
 * member's feed never moves, and a clock inside the census removes the suppression
 * entirely. So the clock lives in neither: the census is unchanged, and the *suppression*
 * expires on a floor. Minor rather than major because nothing a caller reads today
 * changed shape or meaning — two nullable fields appeared, and one config field a network
 * may set.
 *
...
const PEER = {
  ...
  fields: {
    ...
    age: { type: 'number', nullable: true, description: 'Milliseconds on THIS device\'s clock since this device last watched that member\'s log get longer, or null when it has not watched it move at all. The one reading here that answers "am I falling behind on this member": silent and seq answer whether anything ever arrived, which is permanent once true, and this answers when something last did. Never computed from the writer\'s timestamp — that is another device\'s unchecked clock and platform:feed says never to trust it — so it is a duration measured entirely here and is comparable across the members of one reading and not across devices. Null is not zero and not a large number: it means this instance has not seen the log move since it started, which lasts up to one beat floor after a zone restart. And it is only as informative as the beat floor the writing member is running: a member that never writes cannot be observed to have stopped, which is why suppression expires.' }
  }
}
  ...
  operations: [
    {
      ...
      description:
        ...
        'when the census is identical to the last one this device wrote AND that beat is younger than this ' +
        'instance\'s beat floor. The floor is the whole of why a currency reading is possible: pure ' +
        'content-addressed suppression means a healthy member writes nothing, which makes a member that is ' +
        'fine and a member this device stopped replicating with produce byte-identical feeds, so no reading ' +
        'over the feed can separate them. Past the floor an unchanged census is written again — the same ' +
        'numbers, deliberately — so that a member\'s silence is falsifiable. A quiet fleet therefore grows ' +
        'its feed on time as well as on change, bounded at one entry per member per floor; a network that ' +
        'will not pay that sets the floor to zero, gets the old behaviour, and gets a limits() row saying ' +
    ...
    },
    {
      ...
      returns: {
        ...
        fields: {
          ...
          reached: { type: 'number', description: 'Members this device holds at least one entry from. Monotone by construction — a replicated block stays on disk, so a member counted here once is counted here forever, however long ago it arrived. That is the count this field has always named and it is not the answer to "am I falling behind"; peers[].age is' },
          ...
          members: {
            ...
            of: {
              ...
              fields: {
                ...
                at: { type: 'number', description: 'That member\'s own clock when it wrote the beat, as platform:feed carries it on the entry. A display hint and nothing else: it is that device\'s clock, nothing checks it, and it is never a sort key nor the input to any duration here' },
                age: { type: 'number', nullable: true, description: 'Milliseconds on THIS device\'s clock since this device last watched that member\'s log get longer, or null when it has not watched it move. The field to alert on, where at is the field to display — this one is measured here, so it is the only one of the two that means the same thing on every device reading it. See the same field on local().peers for what null means and why it is not zero' },
    ...
    },
    {
      ...
      description:
        ...
        'kind is now a category for most of these names and not for all of them. "zone" is exactly the ' +
        'number of instances whose thread died and "refused" is exactly the number of things this device ' +
        'turned down — a contract nothing satisfies, an instance nothing could deliver, a platform pin this ' +
        'runtime cannot meet, a network retired mid-session, a release below the anti-rollback floor — so ' +
        'some of what it counts is a defence working rather than a fault. "fetch" is failures only, which ' +
        'the platform contract promises rather than merely describes, and it reads zero on any device you ' +
        'can ask this question of: a fetch that fails takes the boot down before an artifact runs, so the ' +
        'zero means this device came up and not that nothing ever failed. "served" is the other half of that ' +
        'split, a release that did arrive from a member rather than its author. "network" is the one name ' +
        'left that is several questions at once — unreachable, unresolved, not yet admitted, a moved pin, an ' +
        'expired wait, a fold that moved on — and is not a count of any one of them. When the port is not ' +
        'bound this answers observed false and no counts at all, rather than zeroes.',
      params: [],
      returns: {
        type: 'object',
    ...
    },
    {
      ...
      description:
        ...
        'shrinks on its own when the platform makes one of these reachable — which it has now done twice, ' +
        'so this list is shorter on a device that binds platform:diagnostics than on one that does not, and ' +
        'a caller comparing two devices is comparing two honest answers rather than one stale one. What is ' +
        'left on it is left for reasons no port can lift: a member that reaches nobody reports to nobody, ' +
        'and a fetch that fails destroys the device before the artifact that would report it exists.',
      params: [],
      returns: {
        type: 'array',
